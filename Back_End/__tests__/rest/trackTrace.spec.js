  ...
  describe("GET /traceer/:trackCode/:verification", () => {
    it("should return a 200", async () => {
      const response = await request.get(
        `${url}/traceer/${data.trackTrace.data.trackTraceId}/1000`
      );
      expect(response.status).toBe(200);
      expect(response.body.companyId).toBe(idForCompany);
      expect(response.body.orderId).toBe(idForOrder);
      expect(response.body.trackTraceId).toBe("A-123456789");
    ...
    it("should return a 404", async () => {
      const response = await request.get(
        `${url}/traceer/${data.trackTrace.data.trackTraceId}/1001`
      );
